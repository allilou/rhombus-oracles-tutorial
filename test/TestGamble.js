...
contract("GambleTestsJS", function (accounts) {
  ...
  it("Write a value into lighthouse", async function () {
    let dataValue = 6;
    let nonce = 1234;
    ...
    var result = await lighthouse.peekData();
    luckyNum = result[0];
    ok = result[1];
    ...
    it("Can send ether", async function(){
      ...
      gamble = await Gamble.deployed();
      
      await gamble.deposit( accounts[0], {value: 7e+6, from: accounts[0] });

      var returned = await gamble.checkBalance( address );
